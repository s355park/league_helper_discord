...
class ModifyMMRCommand(commands.Cog):
    ...
    async def modify_mmr(
        ...
        # Ensure we always respond to avoid timeout
        responded = False
        try:
            ...
            # Last resort error handling - ensure we always respond
            print(f"[Bot] CRITICAL ERROR in modify_mmr: {e}", flush=True)
            import traceback
            traceback.print_exc()
            try:
                ...
                print(f"[Bot] FAILED to send error message: {final_error}", flush=True)
